...
# Paths to schema files - safe for both direct and interactive execution
try:
    # When run as a script directly
    SCHEMA_DIR = Path(__file__).parent
except NameError:
    # When run in Elyra/Jupyter
    SCHEMA_DIR = Path(os.getcwd()) / 'services' / 'neo4j' / 'scripts'
    if not SCHEMA_DIR.exists():
        # Try alternative paths based on current directory structure
        logging.info(f"Trying to find scripts directory from: {os.getcwd()}")
        if Path('scripts').exists():
            SCHEMA_DIR = Path('scripts')
        elif Path('../scripts').exists():
            SCHEMA_DIR = Path('../scripts')
        elif Path.cwd().name == 'scripts':
            SCHEMA_DIR = Path.cwd()
            
    logging.info(f"Using schema directory: {SCHEMA_DIR}")

SCHEMA_FILES = {
    'constraints': SCHEMA_DIR / 'dengue-schema.cypher',
    'nodes': SCHEMA_DIR / 'dengue-nodes.cypher',
    'relationships': SCHEMA_DIR / 'dengue-relationships.cypher'
}

# Paths to data files
DATA_FILES = {
    'snomed': SCHEMA_DIR / 'dengue-snomed.json',
    'obo': SCHEMA_DIR / 'dengue-ontology.obo',
    'owl': SCHEMA_DIR / 'idoden_beta0.15b.owl'
}

# Helper to build path to models/
def model_path(filename):
    """Build path to models directory safely in both execution contexts"""
    try:
        # When run as a script directly
        base_dir = Path(__file__).parent
    except NameError:
        # When run in Elyra/Jupyter
        if SCHEMA_DIR.name == 'scripts':
            base_dir = SCHEMA_DIR
        else:
            base_dir = Path.cwd()
    ...
    # If already an absolute path, return it directly
    if filename.is_absolute():
        return filename
        
    # Handle relative paths
    if '../models' in str(filename):
        return base_dir.parent / 'models' / filename.name
    else:
...
def read_cypher_file(filepath):
    ...
    with open(model_path(filepath), 'r') as f:
        content = f.read()
...
def load_owl_data(driver, filepath):
    ...
    try:
        # Load the ontology
        ontology = owlready2.get_ontology(str(filepath)).load()
