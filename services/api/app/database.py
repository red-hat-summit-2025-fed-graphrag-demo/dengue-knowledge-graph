...
def get_neo4j_driver() -> Driver:
    ...
    try:
        ...
        auth_setting = os.environ.get("NEO4J_AUTH", "basic")
        
        # Create driver with appropriate authentication
        if auth_setting.lower() == "none" or (not NEO4J_USER and not NEO4J_PASSWORD):
            # No authentication - use None for the auth parameter
            logger.info("Using Neo4j connection without authentication")
            ...
            logger.info(f"Using Neo4j connection with basic authentication as user: {NEO4J_USER}")
            driver = GraphDatabase.driver(
                NEO4J_URI,
...
def get_db() -> Generator[Neo4jDatabase, None, None]:
